...
namespace SqliteDbWrapper
{
	...
	public class SqliteDbWrapperFactory : ISqliteDbWrapperFactory
	{
		...
		{
			ICache<string, ICollection<TBaseDbModel>> cache = CreateCache<TBaseDbModel>(pConfigs);
		...
		private static ICache<string, ICollection<TBaseDbModel>> CreateCache<TBaseDbModel>(SqliteDbWrapperConfigs? pConfigs = null)
		{
			if (pConfigs is null || pConfigs.CustomCache is null)
			{
				return CacheFacFactory.CreateStringCacheFactory().BuildCache<string, ICollection<TBaseDbModel>>();
			}

			return pConfigs.CustomCache.BuildCache<string, ICollection<TBaseDbModel>>();
		}

		private static ILogger CreateLogger(Type pCallingType, SqliteDbWrapperConfigs? pConfigs = null)
		{
			if (pConfigs is null || pConfigs.CustomLogger is null)
			{
